...
import { DataTable } from "../components/table/staticDataTable";
...
import staticData from '../projectsData/staticData';
  ...
  const exportToExcel = (projects) => {
    ...
    const headerMap = {
      ...
      estimatedCompletionDate:
        "प्रारंभिक पूर्णता तिथि (एजेंसी द्वारा अनुमानित)",
      ...
      Object.keys(headerMap).forEach((key) => {
        if (key.includes("Date")) {
          rowData[headerMap[key]] = project[key]
            ? new Date(project[key]).toLocaleDateString()
            : "";
        } else if (
          key.includes("Budget") ||
          key.includes("Cost") ||
          ...
          rowData[headerMap[key]] = project[key]
            ? `₹${Number(project[key]).toLocaleString("en-IN")}`
            : "";
        } else {
          rowData[headerMap[key]] = project[key] || "";
        }
    ...
    excelData.forEach((row) => {
      Object.entries(row).forEach(([key, value], i) => {
        const width = Math.max(key.length, value ? value.toString().length : 0);
